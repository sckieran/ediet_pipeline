...
6. set up the scripts and conda environment by typing the following:
   ...
   `cp ./ediet_pipeline/ncbi_lineages*.csv.gz .` #to move the ncbi tax2lin file into your project directory. This is not required, the program will attempt to install a new version of ncbitax2lin if it cannot find this file. However, ncbitax2lin now only works using python 3.7-3.9, most modern systems use python 3.12+. So it is strongly recommended that you either use this file, or install ncbitax2lin outside of the pipeline using a python 3.9 conda environment (you may need to update pandas to get it to run after installation, `pip update pandas` inside the python 3.9 environment).

   
8. add a taxa list for each gene to your project directory. They should be identically named except for the gene name (taxlist_12S, taxlist_ITS, taxlist_fwh1). They can be identical, but don't have to be. Each entry in the taxa list must contain two words, genus and species, separated by a space. If you want to search a whole genus, put the genus name as a single word on its own line (ex. "Microtus"). Do not include a header line.
