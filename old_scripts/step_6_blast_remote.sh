...
echo "You set remote to TRUE, so now doing remote blast search, this may take many hours. You have $n sequences to align. You can check your progress in the ${gene}_out folder with the command: tail ${prefix}_${gene}_raw_blast_out. Then the program will assign taxa to each hit, which will also be slow. Plan to run this overnight for 1000-5000 sequences, or over 2-3 days for 5000+ sequences."
    		...
    		while true;
	 		...
	 		then
	 			echo "outfile for $fil does not yet exist or is empty. Doing $fil."
     				res=$(sbatch ${dirr}/scripts/run_tax_remote_full.sh $x $prefix $gene $tot_per_file $blastout $ncbi $dirr)
