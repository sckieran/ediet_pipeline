...
cd ${dir}/${gene}_out
echo -n "" > remote_${prefix}_${gene}_best_blast_hits.out_${x}
while read p;
	do
		sqy=$( grep -w -A1 "$p" ${prefix}_${gene}_combined_ASVs.fasta | tail -n1 | awk '{print $1}')
  		if cat remote_list_of_no_hits | grep -w -q "${p}"
		then
			echo "no hit found by BLAST for ${p}".
			echo "${sqy}	${p}	0	No Hit	NA	NA	NA	NA	NA	NA	0	0	NA" >> remote_${prefix}_${gene}_best_blast_hits.out_${x}
		else
			...
			top_id2=$( sort -k3 -nr remote_temp_choose2_${x} | cut -f3 | head -n1 | awk '{print $1}')
			...
			if [[ $n -eq 1 ]] 
			then	
				ln="head -n1 remote_temp_seq_${x}"
				num_tx=1
				spec=$( head -n1 remote_temp_seq_${x} | cut -f5 | awk '{print $1,$2}')
				lin=$($ln)
				st=$(echo $lin | awk -v OFS='\t' '{print $1,$3,$5" "$6,$7}')
				taxid=$(echo $lin | awk '{print $7}')
				tax=$(grep -w "^${taxid}" ${ncbi}_r | awk -v OFS='\t' '{print $2,$3,$4,$5,$6}')
				echo "${sqy}	${st}	${tax}	${top_score}	${num_tx}	${spec}" >> remote_${prefix}_${gene}_best_blast_hits.out_${x}
				rm remote_temp_seq_${x}
			elif [[ $n -gt 1 ]] 
			then
				echo "more than one equally-good blast hit available. Walking up the taxonomy tree to reach consensus."
				id=$(cut -f3 remote_temp_choose2_${x} | sort -nr |  head -n1 | awk '{print $1}')
				grep "$id" remote_temp_choose2_${x} > remote_temp_choose_${x} ##pull all the matches with the top identity # for a sequence into a new file##
				...
				if [[ $spec_number -eq 1 ]]
				then
					spec=$(head -n1 remote_temp_choose_${x} |cut -f5 | awk '{print $1,$2}') 
					echo "one species amongst best hits. species is ${spec}."
					ln="head -n1 remote_temp_choose_${x}"
					lin=$($ln)
					st=$(echo $lin | awk -v OFS='\t' '{print $1,$3,$5" "$6,$7}')
					taxid=$(echo $lin | awk '{print $7}')
					tax=$(grep -w "^${taxid}" ${ncbi}_r | awk -v OFS='\t' '{print $2,$3,$4,$5,$6}')	
					...
					do
						...
						grep -w "^${taxid}" ${ncbi}_r | cut -f2-6 >> remote_temp_ids_${x}
					done < remote_temp_choose_${x}
					paste remote_temp_choose_${x} remote_temp_ids_${x} > remote_temp_tax_${x}
					rm remote_temp_choose_${x} remote_temp_ids_${x}
					gen_number=$( cut -f12 remote_temp_tax_${x} | sort | uniq | wc -l)
					fam_number=$( cut -f11 remote_temp_tax_${x} | sort | uniq | wc -l)
					order_number=$( cut -f10 remote_temp_tax_${x} | sort | uniq | wc -l)
					class_number=$( cut -f9 remote_temp_tax_${x} | sort | uniq | wc -l)
					phylum_number=$( cut -f8 remote_temp_tax_${x} | sort | uniq | wc -l)
				fi
				if [[ ${spec_number} -gt 1 ]] && [[ $gen_number -eq 1 ]]
				then
					gen=$( cut -f12 remote_temp_tax_${x} | sort | uniq | awk '{print $1}')
					echo "one genus and ${spec_number} species amongst best hits. taxa is ${gen} sp."
					ln="grep -m1 "$gen" remote_temp_tax_${x}"
					lin=$($ln)
				...
				then
					...
					ln="grep -m1 "$fam" remote_temp_tax_${x}"
					lin=$($ln)
					st=$(echo $lin | awk -v OFS='\t' '{print $1,$3}')
				...
				then
					...
					ln="grep -m1 "$ord" remote_temp_tax_${x}"
					lin=$($ln)
					st=$(echo $lin | awk -v OFS='\t' '{print $1,$3}')
				...
				then
					...
					ln="grep -m1 "$class" remote_temp_tax_${x}"
					lin=$($ln)
					st=$(echo $lin | awk -v OFS='\t' '{print $1,$3}')
				...
				then
					...
					ln="grep -m1 "$phylum" remote_temp_tax_${x}"
					lin=$($ln)
					st=$(echo $lin | awk -v OFS='\t' '{print $1,$3}')
 ...
 rm remote_temp_*_${x}
 echo "job remote_seqlist_${x} is done"
